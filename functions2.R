...
#optimise 2nd stage based on S2 values (using En2)
...
Two.stage.design <- function(msk, alltraps, L0, Sig, Dens, ntrps, nfixed, ngens, nreps){
  ...
  for (i in 1:nreps){
    ...
    opt <- GAoptim(msk, alltraps = remtrps_sf, fixedtraps = fixedtrps_sf, ntraps = ntrps,
                   ...
                   detectfn = 'HHN', D = Dens, noccasions = 1, ngen = ngens, verbose = 0, criterion = 5)
...
## a later version of GAoptim, adapted by Ian Durbach,2022-10-02,06,22
## to use with 2stage design with fixed detectors from stage 1
##############################################################################
