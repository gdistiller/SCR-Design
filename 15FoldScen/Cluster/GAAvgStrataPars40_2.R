...
#Using avg values
#version 2 uses a higher ngen value
...
GA4.AvgStrataPars2.40.list <- vector("list", nreps)
GA5.AvgStrataPars2.40.list <- vector("list", nreps)

#using foreach 
GA4.AvgStrataPars2.40.list <- foreach (r = 1:nreps, .combine='c') %dopar% {Proposed.traps(poly = NULL, alltraps = trap.locs, mask.buff = mask, D = mean(D), sigma = mean(sigma), 
                                                                  lambda0 = mean(L0), sigma.buff = sigma1, grid.spacing = NULL, criterion = 4, 
                                                                  n.reps = 1, grid = FALSE, nT = nT, GA.ngen = 500)}

GA5.AvgStrataPars2.40.list <- foreach (r = 1:nreps, .combine='c') %dopar% {Proposed.traps(poly = NULL, alltraps = trap.locs, mask.buff = mask, D = mean(D), sigma = mean(sigma), 
                                                                  lambda0 = mean(L0), sigma.buff = sigma1, grid.spacing = NULL, criterion = 5, 
                                                                  n.reps = 1, grid = FALSE, nT = nT, GA.ngen = 500)}

save.image("GAAvgStrataPars2.RData")
