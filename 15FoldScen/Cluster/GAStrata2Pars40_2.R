...
GA4.Strata2Pars2.40.list <- vector("list", nreps)
GA5.Strata2Pars2.40.list <- vector("list", nreps)

#using foreach 
GA4.Strata2Pars2.40.list <- foreach (r = 1:nreps, .combine='c') %dopar% {Proposed.traps(poly = NULL, alltraps = trap.locs, mask.buff = mask, D = D2, sigma = sigma2, 
                                                                  lambda0 = L02, sigma.buff = sigma2, grid.spacing = NULL, criterion = 4, 
                                                                  n.reps = 1, grid = FALSE, nT = nT, GA.ngen = 500)}

GA5.Strata2Pars2.40.list <- foreach (r = 1:nreps, .combine='c') %dopar% {Proposed.traps(poly = NULL, alltraps = trap.locs, mask.buff = mask, D = D2, sigma = sigma2, 
                                                                  lambda0 = L02, sigma.buff = sigma2, grid.spacing = NULL, criterion = 5, 
                                                                  n.reps = 1, grid = FALSE, nT = nT, GA.ngen = 500)}

save.image("GAStrata2Pars2.RData")
