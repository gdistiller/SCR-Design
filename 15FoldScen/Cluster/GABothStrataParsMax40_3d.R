#October 2025, code to generate GA designs with 40 traps on 45km regular area
...
#this the 2nd set, file called d and saving as GA5..._b (dont get confused with script name)
...
#GA4.BothStrataParsMax3.40.list <- vector("list", nreps)
GA5.BothStrataParsMax3.40.list <- vector("list", nreps)

#using foreach 
#GA4.BothStrataParsMax3.40.list <- foreach (r = 1:nreps, .combine='c') %dopar% {Proposed.traps(poly = NULL, alltraps = trap.locs, mask.buff = mask, D = D, sigma = sigma, 
#                                                                  lambda0 = L0, sigma.buff = sigma1, grid.spacing = NULL, criterion = 4, 
#                                                                  n.reps = 1, grid = FALSE, nT = nT, GA.ngen = 500, maxim = TRUE)}
#save.image("GABothStrataParsMax3.RData")
#renamed to be GA4BothStrataParsMax3.RData

GA5.BothStrataParsMax3.40.list <- foreach (r = 1:nreps, .combine='c') %dopar% {Proposed.traps(poly = NULL, alltraps = trap.locs, mask.buff = mask, D = D, sigma = sigma, 
                                                                  lambda0 = L0, sigma.buff = sigma1, grid.spacing = NULL, criterion = 5, 
                                                                  n.reps = 1, grid = FALSE, nT = nT, GA.ngen = 500, maxim = TRUE)}

save.image("GA5BothStrataParsMax3b.RData")
