...
#Using values from both strata with 0.5 weights
#uses higher ngen
...
#GA4.BothStrataPars3.40.list <- vector("list", nreps)
GA5.BothStrataPars3.40.list <- vector("list", nreps)

#using foreach 
#GA4.BothStrataPars3.40.list <- foreach (r = 1:nreps, .combine='c') %dopar% {Proposed.traps(poly = NULL, alltraps = trap.locs, mask.buff = mask, D = D, sigma = sigma, 
#                                                                  lambda0 = L0, sigma.buff = sigma1, grid.spacing = NULL, criterion = 4, 
#                                                                  n.reps = 1, grid = FALSE, nT = nT, GA.ngen = 500)}
#save.image("GABothStrataPars3.RData")
#renamed to be GA4BothStrataPars3

GA5.BothStrataPars3.40.list <- foreach (r = 1:nreps, .combine='c') %dopar% {Proposed.traps(poly = NULL, alltraps = trap.locs, mask.buff = mask, D = D, sigma = sigma, 
                                                                  lambda0 = L0, sigma.buff = sigma1, grid.spacing = NULL, criterion = 5, 
                                                                  n.reps = 1, grid = FALSE, nT = nT, GA.ngen = 500)}

save.image("GA5BothStrataPars3b.RData")
