...
nT <- 40 ; nreps <- 500 ; cores <- 30

#using optimal spacings of 500 / 1000
mask.os <- SCR.objs$'OS (40'[[1]]
...
Grid.os.Data <- Sim.data(scenario.df = scen, nreps = 1, traplist = grid.designs.40$`Cluster (opt)`, masklist = mask.os)
Grid.os.results <- Fit.models(Data.obj = Grid.os.Data, mask = mask.os, nrep = nreps, numcores = cores)

save.image("OSclusters40.RData")
