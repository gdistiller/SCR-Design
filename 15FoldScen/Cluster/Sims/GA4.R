#July 2025, simulations to evaluate different proposed GA4 designs for 15 fold scen
...
GA4.S1.results <- Sim.designs(scenario.df = scen, nreps = 1, traplist = GA.designs$G4S1, masklist = mask, raw = TRUE, numcores = 20)
save.image("GA4Results240.RData")

GA4.S2.results <- Sim.designs(scenario.df = scen, nreps = 1, traplist = GA.designs$G4S2, masklist = mask, raw = TRUE, numcores = 20)
save.image("GA4Results240.RData")

GA4.Avg.results <- Sim.designs(scenario.df = scen, nreps = 1, traplist = GA.designs$G4Avg, masklist = mask, raw = TRUE, numcores = 20)
save.image("GA4Results240.RData")

GA4.Both.results <- Sim.designs(scenario.df = scen, nreps = 1, traplist = GA.designs$G4Both, masklist = mask, raw = TRUE, numcores = 20)
save.image("GA4Results240.RData")

GA4.BothMax.results <- Sim.designs(scenario.df = scen, nreps = 1, traplist = GA.designs$G4BothMax, masklist = mask, raw = TRUE, numcores = 20)
save.image("GA4Results240.RData")
