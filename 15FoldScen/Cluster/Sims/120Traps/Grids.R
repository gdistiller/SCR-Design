#May 2026, simulations to evaluate grid designs for 15 fold scen, for 120 traps
...
Grid.800.Data <- Sim.data(scenario.df = scen, nreps = 1, traplist = grid.designs.120$`800 m`, masklist = mask)
system.time(Grid.800.results <- Fit.models(Data.obj = Grid.800.Data, nrep = nreps, numcores = cores))
save.image("GridsResults120.RData")

#Grid.1600.Data <- Sim.data(scenario.df = scen, nreps = 1, traplist = grid.designs.120$`Avg sigma`, masklist = mask)
#system.time(Grid.1600.results <- Fit.models(Data.obj = Grid.1600.Data, nrep = nreps, numcores = cores))
#save.image("GridsResults120.RData")

Grid.700.Data <- Sim.data(scenario.df = scen, nreps = 1, traplist = grid.designs.120$`700 m (2G opt)`, masklist = mask)
system.time(Grid.700.results <- Fit.models(Data.obj = Grid.700.Data, nrep = nreps, numcores = cores))
save.image("GridsResults120.RData")
