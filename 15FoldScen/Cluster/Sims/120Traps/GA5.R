...
#and XXX2 is 2nd set of 400, changed seed to 2026
...
G5.S1.Dataa2 <- Sim.data(scenario.df = scen, nreps = 1, traplist = GA.designs.120$G5S1, masklist = mask)
G5.S1.resultsa2 <- Fit.models(Data.obj = G5.S1.Dataa2, nrep = nreps, numcores = cores)
save.image("GA5Results120a2.RData")

G5.S2.Datab2 <- Sim.data(scenario.df = scen, nreps = 1, traplist = GA.designs.120$G5S2, masklist = mask)
G5.S2.resultsb2 <- Fit.models(Data.obj = G5.S2.Datab2, nrep = nreps, numcores = cores)
save.image("GA5Results120b2.RData")

#G5.Avg.Datac <- Sim.data(scenario.df = scen, nreps = 1, traplist = GA.designs.120$G5Avg, masklist = mask)
#G5.Avg.resultsc <- Fit.models(Data.obj = G5.Avg.Datac, nrep = nreps, numcores = cores)
#save.image("GA5Results120c.RData")

#G5.Both.Data <- Sim.data(scenario.df = scen, nreps = 1, traplist = GA.designs.120$G5Both, masklist = mask)
#G5.Both.results <- Fit.models(Data.obj = G5.Both.Data, nrep = nreps, numcores = cores)
#save.image("GA5Results120.RData")

#G5.BothMax.Data <- Sim.data(scenario.df = scen, nreps = 1, traplist = GA.designs.120$G5BothMax, masklist = mask)
