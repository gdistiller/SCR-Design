...
Grid.550.Data <- Sim.data(scenario.df = scen, nreps = 1, traplist = grid.designs.120$`550 m`, masklist = mask)
Grid.550.results <- Fit.models(Data.obj = Grid.550.Data, nrep = nreps, numcores = cores)
save.image("GridseResults120.RData")

Grid.600.Data <- Sim.data(scenario.df = scen, nreps = 1, traplist = grid.designs.120$`600 m`, masklist = mask)
