#May 26, simulations to evaluate clustered designs for 15 fold scen, for 120 traps
##this version does 500 reps for one config and uses 40 cores
#proposed designs are in the GridDesigns obj
...
#first using optimal spacings of 500 / 1200
mask.os <- SCR.objs$OS[[1]]

system.time(Grid.os.Data <- Sim.data(scenario.df = scen, nreps = 1, traplist = grid.designs.120$`Cluster (opt)`, masklist = mask.os))
system.time(Grid.os.results <- Fit.models(Data.obj = Grid.os.Data, mask = mask.os, nrep = nreps, numcores = cores))

save.image("Clusters120.RData")

#2 sigma pacing
mask.2sig <- SCR.objs$`2 sig`[[1]]

system.time(Grid.2sig.Data <- Sim.data(scenario.df = scen, nreps = 1, traplist = grid.designs.120$`Cluster (2 sig)`, masklist = mask.2sig))
system.time(Grid.2sig.results <- Fit.models(Data.obj = Grid.2sig.Data, mask = mask.2sig, nrep = nreps, numcores = cores))
save.image("Clusters120.RData")
