#27 July 26, simulations to evaluate optimal spacing grid designs (2G) for 15 fold scen
#redoing with new spacing for 120 traps
#job failed 26 July so am breaking up into 5 separate jobs with 30 cores each
...
Grid.1300.Data <- Sim.data(scenario.df = scen, nreps = 1, traplist = grid.designs.120$`1300 m`[301:400], masklist = mask)
Grid.1300.results <- Fit.models(Data.obj = Grid.1300.Data, nrep = nreps, numcores = cores)

save.image("OSGridsResults120d.RData")
