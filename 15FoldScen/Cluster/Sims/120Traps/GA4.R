#April 2026, simulations to evaluate different proposed GA4 designs for 15 fold scen, for 120 traps
...
#this version does 500 reps and uses 40 cores
#actually breakng into bits, a - e
#and XXX2 is the set set of 400 reps, changed seed to 2026
...
G4.S1.Dataa2 <- Sim.data(scenario.df = scen, nreps = 1, traplist = GA.designs.120$G4S1, masklist = mask)
G4.S1.resultsa2 <- Fit.models(Data.obj = G4.S1.Dataa2, nrep = nreps, numcores = cores)
save.image("GA4Results120a2.RData")

G4.S2.Datab2 <- Sim.data(scenario.df = scen, nreps = 1, traplist = GA.designs.120$G4S2, masklist = mask)
G4.S2.resultsb2 <- Fit.models(Data.obj = G4.S2.Datab2, nrep = nreps, numcores = cores)
save.image("GA4Results120b2.RData")

#G4.Avg.Datac <- Sim.data(scenario.df = scen, nreps = 1, traplist = GA.designs.120$G4Avg, masklist = mask)
#G4.Avg.resultsc <- Fit.models(Data.obj = G4.Avg.Datac, nrep = nreps, numcores = cors)
#save.image("GA4Results120c.RData")

#G4.Both.Data <- Sim.data(scenario.df = scen, nreps = 1, traplist = GA.designs.120$G4Both, masklist = mask)
#G4.Both.results <- Fit.models(Data.obj = G4.Both.Data, nrep = nreps, numcores = cores)
#save.image("GA4Results120.RData")

#G4.BothMax.Data <- Sim.data(scenario.df = scen, nreps = 1, traplist = GA.designs.120$G4BothMax, masklist = mask)
