#July 26, simulations to evaluate clustered designs for 15 fold scen
#new version uses different spacings for OS after OS update
...
nT <- 120 ; nreps <- 500 ; cores <- 30

#using optimal spacings of 600 / 1700
mask.os <- SCR.objs$'OS (120'[[1]]
...
Grid.os.Data <- Sim.data(scenario.df = scen, nreps = 1, traplist = grid.designs.120$`Cluster (opt)`, masklist = mask.os)
Grid.os.results <- Fit.models(Data.obj = Grid.os.Data, mask = mask.os, nrep = nreps, numcores = cores)

save.image("OSclusters120.RData")
