...
#version 2 includes higher ngen
...
GA4.Strata1Pars2.40.list <- vector("list", nreps)
GA5.Strata1Pars2.40.list <- vector("list", nreps)

#using foreach 
GA4.Strata1Pars2.40.list <- foreach (r = 1:nreps, .combine='c') %dopar% {Proposed.traps(poly = NULL, alltraps = trap.locs, mask.buff = mask, D = D1, sigma = sigma1, 
                                                                  lambda0 = L01, sigma.buff = sigma1, grid.spacing = NULL, criterion = 4, 
                                                                  n.reps = 1, grid = FALSE, nT = nT, GA.ngen = 500)}

GA5.Strata1Pars2.40.list <- foreach (r = 1:nreps, .combine='c') %dopar% {Proposed.traps(poly = NULL, alltraps = trap.locs, mask.buff = mask, D = D1, sigma = sigma1, 
                                                                  lambda0 = L01, sigma.buff = sigma1, grid.spacing = NULL, criterion = 5, 
                                                                  n.reps = 1, grid = FALSE, nT = nT, GA.ngen = 500)}

save.image("GAStrata1Pars2.RData")
