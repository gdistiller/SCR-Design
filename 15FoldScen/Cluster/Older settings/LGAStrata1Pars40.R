#Oct 2024, code to generate GA designs with 40 traps on large regular area
#requires the following objects: mask, traplocs
#Using strata 1 values (small sigma, larger D)
...
D1 = 0.0012 ; D2 = D1 / DiffFactor  ; D <- c(D1, D2)
lambda0 = 1
nT <- 40 ; nreps = 500
...
L.GA4.Strata1Pars.40.list <- vector("list", nreps)
L.GA5.Strata1Pars.40.list <- vector("list", nreps)

#using foreach 
L.GA4.Strata1Pars.40.list <- foreach (r = 1:nreps, .combine='c') %dopar% {Proposed.traps(poly = NULL, alltraps = trap.locs, mask.buff = mask, D = D1, sigma = sigma1, 
                                                                  lambda0 = lambda0, sigma.buff = sigma1, grid.spacing = NULL, criterion = 4, 
                                                                  n.reps = 1, grid = FALSE, nT = nT, GA.ngen = 50)}

L.GA5.Strata1Pars.40.list <- foreach (r = 1:nreps, .combine='c') %dopar% {Proposed.traps(poly = NULL, alltraps = trap.locs, mask.buff = mask, D = D1, sigma = sigma1, 
                                                                  lambda0 = lambda0, sigma.buff = sigma1, grid.spacing = NULL, criterion = 5, 
                                                                  n.reps = 1, grid = FALSE, nT = nT, GA.ngen = 50)}

save.image("LGAStrata1Pars.RData")
