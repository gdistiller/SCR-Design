...
#Using strata 2 values (large sigma, small D)
...
L.GA4.Strata2Pars.40.list <- vector("list", nreps)
L.GA5.Strata2Pars.40.list <- vector("list", nreps)

#using foreach 
L.GA4.Strata2Pars.40.list <- foreach (r = 1:nreps, .combine='c') %dopar% {Proposed.traps(poly = NULL, alltraps = trap.locs, mask.buff = mask, D = D2, sigma = sigma2, 
                                                                  lambda0 = lambda0, sigma.buff = sigma2, grid.spacing = NULL, criterion = 4, 
                                                                  n.reps = 1, grid = FALSE, nT = nT, GA.ngen = 50)}

L.GA5.Strata2Pars.40.list <- foreach (r = 1:nreps, .combine='c') %dopar% {Proposed.traps(poly = NULL, alltraps = trap.locs, mask.buff = mask, D = D2, sigma = sigma2, 
                                                                  lambda0 = lambda0, sigma.buff = sigma2, grid.spacing = NULL, criterion = 5, 
                                                                  n.reps = 1, grid = FALSE, nT = nT, GA.ngen = 50)}

save.image("LGAStrata2Pars.RData")
