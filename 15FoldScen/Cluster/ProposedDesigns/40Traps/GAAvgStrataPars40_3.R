...
#GA4.AvgStrataPars3.40.list <- vector("list", nreps)
GA5.AvgStrataPars3.40.list <- vector("list", nreps)

#using foreach 
#GA4.AvgStrataPars3.40.list <- foreach (r = 1:nreps, .combine='c') %dopar% {Proposed.traps(poly = NULL, alltraps = trap.locs, mask.buff = mask, D = mean(D), sigma = mean(sigma), 
#                                                                  lambda0 = mean(L0), sigma.buff = sigma1, grid.spacing = NULL, criterion = 4, 
#                                                                  n.reps = 1, grid = FALSE, nT = nT, GA.ngen = 500)}
#save.image("GAAvgStrataPars3.RData")
#renamed to be GA4AvgStrataPars3.RData

GA5.AvgStrataPars3.40.list <- foreach (r = 1:nreps, .combine='c') %dopar% {Proposed.traps(poly = NULL, alltraps = trap.locs, mask.buff = mask, D = mean(D), sigma = mean(sigma), 
                                                                  lambda0 = mean(L0), sigma.buff = sigma1, grid.spacing = NULL, criterion = 5, 
                                                                  n.reps = 1, grid = FALSE, nT = nT, GA.ngen = 500)}

save.image("GA5AvgStrataPars3.RData")
