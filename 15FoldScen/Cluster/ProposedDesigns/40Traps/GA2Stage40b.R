#June 26, code to generate 2 stage designs with 40 traps on 45km regular area
#version b here uses foreach properly after realising I had not done so
...
#extended GAoptim
GAoptim <- function(mask, alltraps, ntraps, fixedtraps = NULL, detectpar, noccasions, detectfn  = c("HHN", "HHR", "HEX", "HAN", "HCG"), D = NULL, criterion = 4, penalty = NULL, seed = NULL, ...){
...
#note that these were ran with ".combine = 'c' which makes a long vector, using .combine = 'list' would have been preferable

TwoStage.40.list <- vector("list", nreps)

TwoStage.40.list <- foreach (r = 1:nreps, .combine='c') %dopar% {Two.stage.design(msk = mask, alltraps = trap.locs, L0 = L01, Sig = sigma1, Dens = D1, ntrps = nT, nfixed = nT/2, ngens = 500)}

save.image("GA2Stage40b.RData")
