#April 26 code to generate GA5 designs with 120 traps on 45km regular area
#requires the following objects: mask, traplocs
#using foreach and doMPI to get 500 designs, but due to timing doing 250 in this job and naming the results obj as a)
#Using values from both strata (Max)
#version b) doing 2nd set of 250 reps
...
GA5.BothStrataParsMax.120.listb <- vector("list", nreps)

#using foreach 
GA5.BothStrataParsMax.120.listb <- foreach (r = 1:nreps, .combine='c') %dopar% {Proposed.traps(poly = NULL, alltraps = trap.locs, mask.buff = mask, D = D, sigma = sigma, 
                                                                  lambda0 = L0, sigma.buff = sigma1, grid.spacing = NULL, criterion = 5, 
                                                                  n.reps = 1, grid = FALSE, nT = nT, GA.ngen = 500, maxim = TRUE)}

save.image("GA5BothStrataParsMax120b.RData")
