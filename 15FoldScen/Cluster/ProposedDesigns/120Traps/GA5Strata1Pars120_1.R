...
GA5.Strata1Pars.40.list <- vector("list", nreps)

#using foreach 
GA5.Strata1Pars.40.list <- foreach (r = 1:nreps, .combine='c') %dopar% {Proposed.traps(poly = NULL, alltraps = trap.locs, mask.buff = mask, D = D1, sigma = sigma1, 
                                                                  lambda0 = L01, sigma.buff = sigma1, grid.spacing = NULL, criterion = 5, 
                                                                  n.reps = 1, grid = FALSE, nT = nT, GA.ngen = 500)}

save.image("GA5Strata1Pars.RData")
