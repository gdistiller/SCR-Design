...
GA4.AvgStrataPars.120.list <- vector("list", nreps)

#using foreach 
GA4.AvgStrataPars.120.list <- foreach (r = 1:nreps, .combine='c') %dopar% {Proposed.traps(poly = NULL, alltraps = trap.locs, mask.buff = mask, D = mean(D), sigma = mean(sigma), 
                                                                  lambda0 = mean(L0), sigma.buff = sigma1, grid.spacing = NULL, criterion = 4, 
                                                                  n.reps = 1, grid = FALSE, nT = nT, GA.ngen = 500)}

save.image("GA4AvgStrataPars.RData")
