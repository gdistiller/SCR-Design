...
GA5.Strata2Pars.120.list <- vector("list", nreps)

#using foreach 
GA5.Strata2Pars.120.list <- foreach (r = 1:nreps, .combine='c') %dopar% {Proposed.traps(poly = NULL, alltraps = trap.locs, mask.buff = mask, D = D2, sigma = sigma2, 
                                                                  lambda0 = L02, sigma.buff = sigma2, grid.spacing = NULL, criterion = 5, 
                                                                  n.reps = 1, grid = FALSE, nT = nT, GA.ngen = 500)}

save.image("GA5Strata2Pars.RData")
