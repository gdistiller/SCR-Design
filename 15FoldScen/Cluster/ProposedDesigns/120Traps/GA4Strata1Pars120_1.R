...
GA4.Strata1Pars.120.list <- vector("list", nreps)

#using foreach 
GA4.Strata1Pars.120.list <- foreach (r = 1:nreps, .combine='c') %dopar% {Proposed.traps(poly = NULL, alltraps = trap.locs, mask.buff = mask, D = D1, sigma = sigma1, 
                                                                  lambda0 = L01, sigma.buff = sigma1, grid.spacing = NULL, criterion = 4, 
                                                                  n.reps = 1, grid = FALSE, nT = nT, GA.ngen = 500)}

save.image("GA4Strata1Pars120.RData")
