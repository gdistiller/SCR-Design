...
GA5.BothStrataPars.120.list <- vector("list", nreps)

#using foreach 
GA5.BothStrataPars.120.list <- foreach (r = 1:nreps, .combine='c') %dopar% {Proposed.traps(poly = NULL, alltraps = trap.locs, mask.buff = mask, D = D, sigma = sigma, 
                                                                  lambda0 = L0, sigma.buff = sigma1, grid.spacing = NULL, criterion = 5, 
                                                                  n.reps = 1, grid = FALSE, nT = nT, GA.ngen = 500)}

save.image("GA5BothStrataPars.RData")
