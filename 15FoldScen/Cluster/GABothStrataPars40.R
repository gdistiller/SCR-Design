...
GA4.BothStrataPars.40.list <- vector("list", nreps)
GA5.BothStrataPars.40.list <- vector("list", nreps)

#using foreach 
GA4.BothStrataPars.40.list <- foreach (r = 1:nreps, .combine='c') %dopar% {Proposed.traps(poly = NULL, alltraps = trap.locs, mask.buff = mask, D = D, sigma = sigma, 
                                                                  lambda0 = L0, sigma.buff = sigma1, grid.spacing = NULL, criterion = 4, 
                                                                  n.reps = 1, grid = FALSE, nT = nT, GA.ngen = 50)}

GA5.BothStrataPars.40.list <- foreach (r = 1:nreps, .combine='c') %dopar% {Proposed.traps(poly = NULL, alltraps = trap.locs, mask.buff = mask, D = D, sigma = sigma, 
                                                                  lambda0 = L0, sigma.buff = sigma1, grid.spacing = NULL, criterion = 5, 
                                                                  n.reps = 1, grid = FALSE, nT = nT, GA.ngen = 50)}

save.image("GABothStrataPars.RData")
