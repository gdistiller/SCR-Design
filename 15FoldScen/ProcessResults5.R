#NOv 2025, process sim results for 15 fold scen
...
Grid.800.data.summ <- CH.data.summs(Grid.800.Data$output)
...
##first data summary, then estimates
Grid.1600.data.summ <- CH.data.summs(Grid.1600.Data$output)
...
#filter data, NAs and infinite, and wildly out values (20 fold)
#one strata at a time
Grid.1600.red1 <- find.rogue(Grid.1600.results[,c(1:6)], mag = mag.factor, true = c(0.05, 2, 200))
Grid.1600.red2 <- find.rogue(Grid.1600.results[,c(7:12)], mag = mag.factor, true = c(0.05/15, 2/15, 3000))
...
load("15FoldScen/Cluster/Sims/GA4dResults40.RData")

######################
#GA4 with S1 pars
######################

GA4.S1.data.summ <- CH.data.summs(G4.S1.Data$output)
...
GA4.S2.data.summ <- CH.data.summs(G4.S2.Data$output)
...
GA4.Avg.data.summ <- CH.data.summs(G4.Avg.Data$output)
...
GA4.Both.data.summ <- CH.data.summs(G4.Both.Data$output)
...
GA4.BothMax.data.summ <- CH.data.summs(G4.BothMax.Data$output)
...
GA5.S1.data.summ <- CH.data.summs(G5.S1.Data$output)
...
GA5.S2.data.summ <- CH.data.summs(G5.S2.Data$output)
...
GA5.Avg.data.summ <- CH.data.summs(G5.Avg.Data$output)
...
GA5.Both.data.summ <- CH.data.summs(G5.Both.Data$output)
...
GA5.BothMax.data.summ <- CH.data.summs(G5.BothMax.Data$output)
