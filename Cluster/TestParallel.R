#Sep 2024, testing parallel computing
#code to generate GA designs on cluster with 40 traps
...
#foreach(r = 1:i, .combine = 'c' / 'cbind' / ')

system.time(TestResults <- Proposed.traps(poly = NULL, alltraps = trap.locs, mask.buff = mask, D = DF, sigma = sigmaF, lambda0 = lambda0F, sigma.buff = sigmaF, grid.spacing = NULL, criterion = 4, n.reps = 5, 
                                          grid = FALSE, nT = nT, GA.ngen = 10))

system.time(TestResults2 <- foreach (r = 1:5, .combine='c') %dopar% {Proposed.traps(poly = NULL, alltraps = trap.locs, mask.buff = mask, D = DF, sigma = sigmaF, 
                                                                      lambda0 = lambda0F, sigma.buff = sigmaF, grid.spacing = NULL, criterion = 4, n.reps = 1, 
                                                                      grid = FALSE, nT = nT, GA.ngen = 10)})
