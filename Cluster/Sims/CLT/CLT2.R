...
fit.all.null <- secr.fit(CH, mask = CLTsimp.nobuff, hcov = "Sex",list(D ~ 1, lambda0 ~ 1, sigma ~ 1), detectfn = 'HHN')
...
#by section
fit.N.null <- secr.fit(CH.Northern, mask = CLTsimp.nobuff,hcov = "Sex", list(D ~ 1, lambda0 ~ 1, sigma ~ 1), detectfn = 'HHN')
fit.N.sex <- secr.fit(CH.Northern, mask = CLTsimp.nobuff, hcov = "Sex", list(D ~ 1, lambda0 ~ h2, sigma ~ h2),  detectfn = 'HHN')

save.image("CLTResults.RData")

fit.S.null <- secr.fit(CH.Southern, mask = CLTsimp.nobuff,hcov = "Sex", list(D ~ 1, lambda0 ~ 1, sigma ~ 1), detectfn = 'HHN')
