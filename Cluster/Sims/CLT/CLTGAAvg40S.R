...
mask <- CLTmsks$`Mask of South` ; trap.locs <- CLTmsks$`Trap locs for South`
...
GA.Avg4.traplist <- vector("list", nreps)
GA.Avg5.traplist <- vector("list", nreps)

#using foreach 
GA.Avg4.traplist <- foreach (r = 1:nreps, .combine='c') %dopar% {Proposed.traps(poly = NULL, alltraps = trap.locs, mask.buff = mask, D = mean(D), sigma = mean(sigma), 
                                                         lambda0 = mean(lambda0), sigma.buff = sigmaM, grid.spacing = NULL, criterion = 4, 
                                                         n.reps = 1, grid = FALSE, nT = nT, GA.ngen = 50)}

GA.Avg5.traplist <- foreach (r = 1:nreps, .combine='c') %dopar% {Proposed.traps(poly = NULL, alltraps = trap.locs, mask.buff = mask, D = mean(D), sigma = mean(sigma), 
                                                         lambda0 = mean(lambda0), sigma.buff = sigmaM, grid.spacing = NULL, criterion = 5, 
                                                         n.reps = 1, grid = FALSE, nT = nT, GA.ngen = 50)}

save.image("GAtrapsAvg40S.RData")
