...
#this job is for a grid design with 2 * F sigma
...
#sim function 
Check.design <- function(scenario.df, nreps = 1, traplist, masklist, raw = TRUE, numcores){
...
Grid.FSigma.results <- Check.design(scenario.df = scen, nreps = 1, traplist = grid.designs$`2*F Sigma`, masklist = mask, 
                                    raw = TRUE, numcores = 10)

###################################

save.image("Grid2FSigma.RData")
