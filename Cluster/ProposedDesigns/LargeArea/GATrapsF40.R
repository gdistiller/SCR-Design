#Sep 2024, code to generate GA designs on cluster with 40 traps
...
GA.F4.traplist <- vector("list", nreps)
GA.F5.traplist <- vector("list", nreps)
...
#using foreach 
GA.F4.traplist <- foreach (r = 1:nreps, .combine='c') %dopar% {Proposed.traps(poly = NULL, alltraps = trap.locs, mask.buff = mask, D = DF, sigma = sigmaF, 
                                                                  lambda0 = lambda0F, sigma.buff = sigmaF, grid.spacing = NULL, criterion = 4, 
                                                                  n.reps = 1, grid = FALSE, nT = nT, GA.ngen = 50)}

GA.F5.traplist <- foreach (r = 1:nreps, .combine='c') %dopar% {Proposed.traps(poly = NULL, alltraps = trap.locs, mask.buff = mask, D = DF, sigma = sigmaF, 
                                                                  lambda0 = lambda0F, sigma.buff = sigmaF, grid.spacing = NULL, criterion = 5, 
                                                                  n.reps = 1, grid = FALSE, nT = nT, GA.ngen = 50)}

save.image("GAtrapsF40.RData")
