...
####################################
#get proposed designs using Avg Pars
####################################

L.GA4.AvgSPars.40.list <- vector("list", nreps)
L.GA5.AvgSPars.40.list <- vector("list", nreps)

#using foreach 
L.GA4.AvgSPars.40.list <- foreach (r = 1:nreps, .combine='c') %dopar% {Proposed.traps(poly = NULL, alltraps = trap.locs, mask.buff = mask, D = mean(D), sigma = mean(sigma), 
                                                         lambda0 = mean(lambda0), sigma.buff = sigmaM, grid.spacing = NULL, criterion = 4, 
                                                         n.reps = 1, grid = FALSE, nT = nT, GA.ngen = 50)}

L.GA5.AvgSPars.40.list <- foreach (r = 1:nreps, .combine='c') %dopar% {Proposed.traps(poly = NULL, alltraps = trap.locs, mask.buff = mask, D = mean(D), sigma = mean(sigma), 
                                                         lambda0 = mean(lambda0), sigma.buff = sigmaM, grid.spacing = NULL, criterion = 5, 
                                                         n.reps = 1, grid = FALSE, nT = nT, GA.ngen = 50)}

save.image("LGAAvgSPars40.RData")
