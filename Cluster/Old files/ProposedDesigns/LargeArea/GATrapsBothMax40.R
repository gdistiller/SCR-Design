#Sept 2024, code to generate GA designs on cluster with 40 traps
#requires the following objects: mask, traplocs
#Using both sets of parameter values with Max = T
...
GA.both4Max.traplist <- vector("list", nreps)
GA.both5Max.traplist <- vector("list", nreps)

#using foreach 
GA.both4Max.traplist <- foreach (r = 1:nreps, .combine='c') %dopar% {Proposed.traps(poly = NULL, alltraps = trap.locs, mask.buff = mask, D = D, sigma = sigma, 
                                                                  lambda0 = lambda0, sigma.buff = sigmaM, grid.spacing = NULL, 
                                                                  criterion = 4, n.reps = 1, grid = FALSE, nT = nT, GA.ngen = 50, maxim = TRUE)}

GA.both5Max.traplist <- foreach (r = 1:nreps, .combine='c') %dopar% {Proposed.traps(poly = NULL, alltraps = trap.locs, mask.buff = mask, D = D, sigma = sigma, 
                                                                   lambda0 = lambda0, sigma.buff = sigmaM, grid.spacing = NULL, 
                                                                   criterion = 5, n.reps = 1, grid = FALSE, nT = nT, GA.ngen = 50, maxim = TRUE)}

save.image("GAtrapsBothMax40.RData")
