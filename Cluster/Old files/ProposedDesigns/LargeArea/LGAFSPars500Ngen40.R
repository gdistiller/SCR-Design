#Sept 2024, code to generate GA designs with 40 traps on large regular area
#requires the following objects: mask, traplocs
#Using F values
#using foreach and doMPI to get 500 designs
#rerunning 26 Sept with CLT South values
#increasing ngen to check convergence of GA algo
...
Proposed.traps <- function(poly, alltraps = NULL, nT, D, sigma, lambda0, mask.buff = NULL, sigma.buff = NULL, grid.spacing = NULL, criterion = 4, n.reps = 1, grid = FALSE, GA.ngen = 20, maxim = FALSE, seed = 2024){
...
mask <- scr.objs[[1]] ; trap.locs <- scr.objs[[2]]
...
###################################
#get proposed designs using F sigma
###################################

L.GA4.MSPars.40.list <- vector("list", nreps)
L.GA5.MSPars.40.list <- vector("list", nreps)

#for (i in 1:nreps){
#  GA.F4 <- Proposed.traps(poly = NULL, alltraps = trap.locs, mask.buff = mask, D = DF, sigma = sigmaF, 
...
#  GA.F5 <- Proposed.traps(poly = NULL, alltraps = trap.locs, mask.buff = mask, D = DF, sigma = sigmaF, 
#                          lambda0 = lambda0F, sigma.buff = sigmaF, grid.spacing = NULL, 
...
#using foreach 
L.GA4.MSPars.40.list <- foreach (r = 1:nreps, .combine='c') %dopar% {Proposed.traps(poly = NULL, alltraps = trap.locs, mask.buff = mask, D = DF, sigma = sigmaF, 
                                                                  lambda0 = lambda0F, sigma.buff = sigmaF, grid.spacing = NULL, criterion = 4, 
                                                                  n.reps = 1, grid = FALSE, nT = nT, GA.ngen = 500)}

L.GA5.MSPars.40.list <- foreach (r = 1:nreps, .combine='c') %dopar% {Proposed.traps(poly = NULL, alltraps = trap.locs, mask.buff = mask, D = DF, sigma = sigmaF, 
                                                                  lambda0 = lambda0F, sigma.buff = sigmaF, grid.spacing = NULL, criterion = 5, 
                                                                  n.reps = 1, grid = FALSE, nT = nT, GA.ngen = 500)}

save.image("LGAFSPars500Ngen40.RData")
