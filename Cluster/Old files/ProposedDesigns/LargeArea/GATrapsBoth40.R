...
#Using both sets of parameter values with equal weighting
...
GA.Both4.traplist <- vector("list", nreps)
GA.Both5.traplist <- vector("list", nreps)

#using foreach 
GA.Both4.traplist<- foreach (r = 1:nreps, .combine='c') %dopar% {Proposed.traps(poly = NULL, alltraps = trap.locs, mask.buff = mask, D = D, sigma = sigma, 
                                                                lambda0 = lambda0, sigma.buff = sigmaM, grid.spacing = NULL, 
                                                                criterion = 4, n.reps = 1, grid = FALSE, nT = nT, GA.ngen = 50)}

GA.Both5.traplist <- foreach (r = 1:nreps, .combine='c') %dopar% {Proposed.traps(poly = NULL, alltraps = trap.locs, mask.buff = mask, D = D, sigma = sigma, 
                                                                lambda0 = lambda0, sigma.buff = sigmaM, grid.spacing = NULL, 
                                                                criterion = 5, n.reps = 1, grid = FALSE, nT = nT, GA.ngen = 50)}

save.image("GAtrapsBoth40.RData")
