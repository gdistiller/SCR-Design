...
GA.M4.traplist <- vector("list", nreps)
GA.M5.traplist <- vector("list", nreps)

#using foreach 
GA.M4.traplist <- foreach (r = 1:nreps, .combine='c') %dopar% {Proposed.traps(poly = NULL, alltraps = trap.locs, mask.buff = mask, D = DM, sigma = sigmaM, 
                                                                              lambda0 = lambda0M, sigma.buff = sigmaM, grid.spacing = NULL, 
                                                                              criterion = 4, n.reps = 1, grid = FALSE, nT = nT, GA.ngen = 50)}

GA.M5.traplist <- foreach (r = 1:nreps, .combine='c') %dopar% {Proposed.traps(poly = NULL, alltraps = trap.locs, mask.buff = mask, D = DM, sigma = sigmaM, 
                                                                              lambda0 = lambda0M, sigma.buff = sigmaM, grid.spacing = NULL, 
                                                                              criterion = 5, n.reps = 1, grid = FALSE, nT = nT, GA.ngen = 50)}

save.image("GAtrapsM40.RData")
