...
###################################
#get proposed designs using M sigma
###################################

L.GA4.BothSPars.40.list <- vector("list", nreps)
L.GA5.BothSPars.40.list <- vector("list", nreps)

#using foreach 
L.GA4.BothSPars.40.list <- foreach (r = 1:nreps, .combine='c') %dopar% {Proposed.traps(poly = NULL, alltraps = trap.locs, mask.buff = mask, D = D, sigma = sigma, 
                                                                lambda0 = lambda0, sigma.buff = sigmaM, grid.spacing = NULL, 
                                                                criterion = 4, n.reps = 1, grid = FALSE, nT = nT, GA.ngen = 500)}

L.GA5.BothSPars.40.list <- foreach (r = 1:nreps, .combine='c') %dopar% {Proposed.traps(poly = NULL, alltraps = trap.locs, mask.buff = mask, D = D, sigma = sigma, 
                                                                lambda0 = lambda0, sigma.buff = sigmaM, grid.spacing = NULL, 
                                                                criterion = 5, n.reps = 1, grid = FALSE, nT = nT, GA.ngen = 500)}

save.image("LGABothSPars500Ngen40.RData")
